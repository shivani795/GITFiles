...
def main(dict):
 ...
 if(intent=="know" or intent=="show" or intent=="see" or intent=="get" or intent=="what"):
  ...
  c_no=dict["claimno"]
  url=switch1(intent,c_no)
  data=claimstatuspostResponse(url,c_no)
  print("url",data)
  return {'message': data}
  ...
  c_no=dict["claimno"]
  url=switch1(intent,c_no)
  data=claimhistorypostResponse(url,c_no)
  print("url",data)
  return {'message': data}
  ...
  c_no=dict["claimno"]
  url=switch1(intent,c_no)
  data=claimdownpostResponse(url,c_no)
  print("url",data)
  return {'message': data}
...
def switch1(intent,c_no):
    ...
    return switcher1.get(intent,"Oops! Invalid Option")
...
def claimstatuspostResponse(url,c_no):
  formatdetails3(c_no)
  test=json.dumps(claimstatus_json, indent = 4)
  response =requests.post(url,test,auth=('sbsuperadmin', 'Med2O!sChnTn'),headers=headers,verify=False)
  print("claimstatus_json",response)
  datastr=response.content
  print("claimstatus_json",datastr)
  r= json.loads(datastr)
  return r

def claimhistorypostResponse(url,c_no):
  formatdetails4(c_no)
  test=json.dumps(claimhistory_json, indent = 4)
  response =requests.post(url,test,auth=('sbsuperadmin', 'Med2O!sChnTn'),headers=headers,verify=False)
  print("claimhistory_json",response)
  datastr=response.content
  print("claimhistory_json",datastr)
  r= json.loads(datastr)
  return r

def claimdownpostResponse(url,c_no):
  formatdetails5(c_no)
  test=json.dumps(claimdownload_json, indent = 4)
  response =requests.post(url,test,auth=('sbsuperadmin', 'Med2O!sChnTn'),headers=headers,verify=False)
  print("claimdownload_json",response)
  datastr=response.content
  print("claimdownload_json",datastr)
  r= json.loads(datastr)
  return r
...
def formatdetails3(c_no):
     claimstatus_json["sopExecJSON"]["execSOPParamsData"]["Claimnumber"] =c_no

def formatdetails4(c_no):
     claimhistory_json["sopExecJSON"]["execSOPParamsData"]["Claimnumber"] =c_no

def formatdetails5(c_no):
     claimdownload_json["sopExecJSON"]["execSOPParamsData"]["Claimnumber"] =c_no
